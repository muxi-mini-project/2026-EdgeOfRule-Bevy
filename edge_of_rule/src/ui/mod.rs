...
fn load_assets(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.insert_resource(OptionsImageHandle(asset_server.load("images/options.png")));
    commands.insert_resource(MainFontHandle(asset_server.load("images/font/aLiFont.ttf")));
                            ...
                            slider_container.spawn((
                                NodeBundle {
                                    style: Style {
                                        ...
                                        position_type: PositionType::Absolute,
                                        left: Val::Percent(
                                            volume_settings.volume * 90.0 + 5.0 - 1.2,
                                        ), // 计算手柄位置
                                        ..default()
                                    },
...
fn block_events(mut events: EventReader<CursorMoved>, popup_query: Query<&Node, With<Popup>>) {
    // 如果有弹窗，阻止鼠标移动事件传递
...
fn cleanup_main_menu(mut commands: Commands, menu_query: Query<(Entity, &Name)>) {
